...
// ModifiedBesselFunctionI0(q, DummyQ, SigmaQ) includes also the weighting factors from the Rav resolution function, i.e. not only I0(q*DummyQ/SigmaQ) 
...
int Resolution(struct Dataset * Data, int NumberOfSpectra, char ResolutionFile[256], int NumberOfSmearingFolds, int WriteLog, FILE* logfile)
{
	/// Declarations
	// Dummy variables
	double DummyQ ;
	...
	if ( Inputfile == NULL )
	{
		if ( abs(WriteLog) > 0 )
		{
			fprintf( logfile, "\tAn error occured when attempting to open resolution file\n") ;
			fprintf( logfile, "\n") ;
	...
	while (fgets(Line, sizeof(Line), Inputfile) != NULL)
	{
		if (sscanf(Line, "%d %lf %lf %lf %lf %lf %lf %lf %lf", &DummyInt, &Dummy1, &Dummy2, &Dummy3, &Dummy4, &Dummy5, &Dummy6, &Dummy7, &Dummy8) == 9)
		{
			if (DummyInt != 0)
			{
				...
				if ( abs(WriteLog) > 0 )
				{
					fprintf( logfile, "\t\tIncluding instrumental smearing in fits of dataset %d\n", i + 1) ;
					fprintf( logfile, "\n") ;
	...
	if (i != NumberOfSpectra)
	{
		if ( abs(WriteLog) > 0 )
		{
			fprintf( logfile, "\tThe number of spectra in the .card-file does not match the number of spectra in the .res-file\n") ;
			fprintf( logfile, "\n") ;
	...
	for ( i = 0; i < NumberOfSpectra; ++i)
	{
		if ( Data[i].IncludeResolutionEffects == true )
		{
			...
			for ( j = Data[i].NMin; j < Data[i].NMax; ++j)
			{
				...
				SigmaQ = SigmaOfQ(q, RadiusOfFirstPinhole[i], RadiusOfSecondPinhole[i], CollimationLength[i], SampleToDetectorDistance[i], Wavelength[i], WavelengthSpread[i], DetectorWidth[i], BinningWidth[i]) ;
