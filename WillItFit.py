...
## Define plotting function
    def PlotFnc(self, event):
        ...
        DataArr = []
        FitArr = []
        DirectoryStr = str(self.DataPathStr)+"-results"
        for Filename in os.listdir(DirectoryStr ):
            if os.path.splitext(Filename)[1] == '.dat':
                ShortName = os.path.basename(Filename)

                if ShortName[0:3] == 'fit':
                    FitArr.append(os.path.join(DirectoryStr , ShortName))

                if ShortName[0:3] == 'dat':
                    if ShortName[0:4] != 'data':
                        DataArr.append(os.path.join(DirectoryStr , ShortName))
