...
All call to WIF takes the following arguments:
	...
	-d path to PDB file
	-r choose between computing the model or running the LM algorithm. 0 for computing the model 1 for LM.
 
To call WIF in bash mode, the source code must be compiled first. The following is an example a command used to compile WillItFit.c:
